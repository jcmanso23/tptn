const CACHE_NAME = 'topotino-offline-v39';
const CORE_ASSETS = [
  '/',
  '/index.html',
  '/styles.css?v=memory-v54',
  '/app.js?v=memory-v54',
  '/chat-format.js?v=memory-v54',
  '/content/challenges.js?v=memory-v54',
  '/manifest.json',
  '/icons/icon.svg',
  '/icons/icon-192.png',
  '/icons/icon-512.png',
  ...
  '/content/episodes.json?v=memory-v54'
];
...
async function precacheStory() {
  ...
  try {
    const response = await fetch('/content/episodes.json?v=memory-v54', { cache: 'no-store' });
    if (!response.ok) return;
    const manifest = await response.clone().json();
    await cache.put('/content/episodes.json?v=memory-v54', response);
