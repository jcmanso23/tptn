const CACHE_NAME = 'topotino-offline-v2';
const CORE_ASSETS = [
  '/',
  '/index.html',
  '/styles.css?v=chat-v16',
  '/app.js?v=chat-v16',
  '/manifest.json',
  '/icons/icon.svg',
  '/icons/icon-192.png',
  ...
  '/content/episodes.json?v=chat-v16'
];
...
async function precacheStory() {
  ...
  try {
    const response = await fetch('/content/episodes.json?v=chat-v16', { cache: 'no-store' });
    if (!response.ok) return;
    const manifest = await response.clone().json();
    await cache.put('/content/episodes.json?v=chat-v16', response);
