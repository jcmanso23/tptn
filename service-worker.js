const CACHE_NAME = 'topotino-offline-v8';
const CORE_ASSETS = [
  '/',
  '/index.html',
  '/styles.css?v=memory-v22',
  '/app.js?v=memory-v22',
  '/manifest.json',
  '/icons/icon.svg',
  '/icons/icon-192.png',
  ...
  '/content/episodes.json?v=memory-v22'
];
...
async function precacheStory() {
  ...
  try {
    const response = await fetch('/content/episodes.json?v=memory-v22', { cache: 'no-store' });
    if (!response.ok) return;
    const manifest = await response.clone().json();
    await cache.put('/content/episodes.json?v=memory-v22', response);
